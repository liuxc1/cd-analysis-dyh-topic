...
import com.arronlong.httpclientutil.evictor.HttpClientConnectionMonitorThread;
...
public class HCB extends HttpClientBuilder {
    ...
    private CredentialsProvider hcbcredsProvider = new BasicCredentialsProvider();
...
//		return (HCB) this.setConnectionManager(connManager);
        if (sslConnFactory == null) {
            try {
                SSLContext sslContext = SSLs.getInstance().getSSLContext(SSLProtocolVersion.SSL);
...
//                        , new String[]{"TLSv1", "TLSv1.1", "TLSv1.2", "TLSv1.3"}
        ...
        return (HCB) this.setSSLSocketFactory(sslConnFactory);
    }
...
//		this.isNewSSL=true;
    ...
    public HCB pool(int maxTotal, int defaultMaxPerRoute) throws HttpProcessException {
        ...
        isSetPool = true;
        //清除链接池中过期或者空闲时间超过5分钟的链接，每一分钟执行一次
        HttpClientConnectionMonitorThread clearPoolThread = new HttpClientConnectionMonitorThread(connManager, 60, 300);
        return (HCB) this.setConnectionManager(connManager);
    }
    ...
    /**
     * @return the hcbcredsProvider
     */
    public CredentialsProvider getHcbcredsProvider() {
        return hcbcredsProvider;
    }

    /**
     * 添加证书到HCB，HCB统一添加到HttpClientBuilder
     *
     * @param authscope
     * @param credentials
     * @return
     */
    public CredentialsProvider addCredentials(AuthScope authscope, Credentials credentials) {
        hcbcredsProvider.setCredentials(authscope, credentials);
        this.setDefaultCredentialsProvider(hcbcredsProvider);
        return hcbcredsProvider;
    }
