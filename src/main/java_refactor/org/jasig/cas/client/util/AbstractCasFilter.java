...
public abstract class AbstractCasFilter extends AbstractConfigurationFilter {
    ...
    private volatile boolean casThs = false;
    private String[] allowServerName = null;

    private boolean isAllowServerName(String serverName) {
        if (allowServerName != null) {
            for (String s : allowServerName) {
                if (serverName.contains(s)) {
                    return true;
                }
            }
        }
    ...
    private void resolveCasThs() {
        //初始化
        InputStream is = this.getClass().getClassLoader().getResourceAsStream("cas_ths.properties");
        if (is != null) {
            Properties prop = new Properties();
            try {
                prop.load(is);
                String allowServerNameStr = prop.getProperty("ALLOW_SERVER_NAME");
                if (allowServerNameStr != null && allowServerNameStr.trim().length() > 0) {
                    allowServerName = allowServerNameStr.replace(" ", "").split(",");
                }
                casThs = true;
    ...
    protected final String constructServiceUrl(final HttpServletRequest request, final HttpServletResponse response) {
        ...
        if (casThs) {
            ...
            if (CommonUtils.isNotBlank(host) && isAllowServerName(host)) {
                serverName = host;
            }
        }
