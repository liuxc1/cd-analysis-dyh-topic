...
@Service("DistinctHandleSourceResolveService")
public class DistinctHandleSourceResolveService implements ExcelImportCallbackInterface{
	@Autowired
	...
	public ImportCallbackProp afterCallback(ImportCallbackProp callbackProp) throws Exception {
		...
		try {
			...
			if(file.exists()){
				Map<String, Object> paramMap = handlerExcelSourceResolveToolService.packageCommonFileParameter(variables,props,file,loginUser,AscriptionTypeEnum.DISTINCTSOURCE_ANALYSIS);
				//插入文件信息
				distinctSourceResolveService.insertFile(paramMap);
				//暂时插入报表内容
				Map<String, Object> infosMap = handlerExcelSourceResolveToolService.addreportInfos(paramMap, AscriptionTypeEnum.DISTINCTSOURCE_ANALYSIS);
				distinctSourceResolveService.addReportInfos(infosMap);
				//复制文件到文件夹
				handlerExcelSourceResolveToolService.copyFileToCommonDir(variables, file, props,AscriptionTypeEnum.DISTINCTSOURCE_ANALYSIS);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void forefrontCallback(ImportCallbackProp callbackProp){
		handlerExcelSourceResolveToolService.forebackParameter(callbackProp,AscriptionTypeEnum.DISTINCTSOURCE_ANALYSIS);
	}
