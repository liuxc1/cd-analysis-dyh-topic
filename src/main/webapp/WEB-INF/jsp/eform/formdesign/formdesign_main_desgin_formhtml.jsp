...
<html lang="zh">
	<head>
		...
		<link rel="stylesheet" href="${ctx}/assets/js/eform/eform_custom.css">
		<!--判断是否加载设计模式 css -->
		<c:if test="${isdesign == true }">
			<link rel="stylesheet" href="${ctx}/assets/js/eform/eform_design.css">
		</c:if>
		<!--页面自定义的CSS，请放在这里 -->
		<style>
			.padding_1 {
		...
		<script type="text/javascript" src="${ctx }/assets/components/jquery-ui/jquery-ui.js"></script>
		<!--判断是否加载设计模式 js-->
		<c:if test="${isdesign == true }">
			<script type="text/javascript" src="${ctx}/assets/js/eform/eform_design.js"></script>
		</c:if>
		<!-- webuploader Js -->
