...
<div class="page-toolbar align-right list-toolbar" id="listhtml_btn_toobar">
	<button type="button" class="btn btn-xs    btn-xs-ths"
			id="btnAdd" data-self-js="doEdit('${form.businessKey }');">
...
<script>
	...
	function doEdit(id){
		$.ajax({
			...
			success:function(response){
				if(response=="success"){
					var _url = "${ctx}/eform/formdesign/formdesign_main_desgin_listhtml_edit.vm?formId=${formId}&mainTableCode=${mainTableCode}";
					if(id && id != ""){
						_url += "&businessKey=" + encodeURI(encodeURI(id));
					}
	...
	//查看
	function doView(id){
		var _url = "${ctx}/eform/formdesign/formdesign_main_desgin_listhtml_detail.vm?formId=${formId}&dataSource=${dataSource}&businessKey=" + encodeURI(encodeURI(id));
		<c:forEach items="${form}" var="form" > 
			_url = _url + "&&form['${form.key}']=${form.value}";
		</c:forEach>
		$("#main-container").hide();
