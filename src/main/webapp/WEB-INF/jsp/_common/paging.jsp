...
<div id="zcz_page" class="no-margin-bottom">
    ...
    <div style="position: relative; height: 32px;line-height:32px;float: left;">
        共${pageInfo.pages}页/共${pageInfo.total}条&nbsp
    </div>
    <ul class="page_ul pagination no-margin">
        ...
        <li id="zcz_page_li2" num="up"><a href="javascript:void(0)"><i class="ace-icon fa fa-angle-left"></i></a></li>
    ...
    <div class="page_total pull-right" style="height: 32px; float: left;">
        <div style="position: relative; height: 32px; float: right; text-align: right;">
            <span>
                <select id="pageSize" name="pageSize" onchange="$.Page.submit()">
        	...
        	<input type="text" id="pageChangeNum" value="${pageInfo.pageNum}"  style="width:35px;height:26px; padding: 1px 4px;"
                onKeyUp="$.Page.pageChange(this)"> 页&nbsp
            <button class="btn  btn-xs btn-info btn-xs-ths"  style="width:41px; min-width:41px; max-width:41px; height: 26px; margin-top: -3px;" onclick="$.Page.submit()">GO</button>
        </div>
    </div>
    ...
    <input type="hidden" id="orderBy" name="orderBy" value="${pageInfo.orderBy}">
</div>
<script>
    ...
    function pageTable() {
        ...
        /*if($("#pages").val() == "1")
        	$("#zcz_page").hide();
        */
        //重新赋值，history.go()表单元数值有缓存 add by lixinda 2017-07-20
