...
var DayGrid = /** @class */ (function (_super) {
    ...
    DayGrid.prototype.renderNumberCellHtml = function (date) {
        ...
        html += '<td id="'+date.format()+'" class="' + classes.join(' ') + '"' +
            (isDateValid ?
...
var Calendar = /** @class */ (function () {
    ...
    Calendar.prototype.renderView = function (viewType) {
        ...
        //by wangjp
		if(viewType=="month" && initPageStyle){
			initPageStyle();
		}
    };
...
var DateComponent = /** @class */ (function (_super) {
    ...
    DateComponent.prototype.buildGotoAnchorHtml = function (gotoOptions, attrs, innerHtml) {
        ...
        innerHtml = innerHtml || '';
        //modify by wangjp
    	if(lunar && this.name=="month"){
			var _month=(""+(date._d.getMonth()+1)).length==1?"0"+(date._d.getMonth()+1):date._d.getMonth()+1;
			var _d=(""+date._d.getDate()).length==1?"0"+date._d.getDate():date._d.getDate();
			var _date=""+date._d.getFullYear()+_month+_d;
			//获取农历的月
			var lmonth=lunar(date).lMonth;
			//获取农历的日
			var ldate=lunar(date).lDate;
			if(ldate=='初一'){
				ldate=lmonth+"月";
			}
			//获取节气
			...
			var middle;
			//如果这一天既是节日又是节气,就按照‘节日/节气’的形式显示
			if(cTerm && fes.length>0){
				middle=fes[0].desc +"/"+cTerm;
			}else if(cTerm){
				middle=cTerm;
			}else if(fes && fes.length>0 ){
				middle=fes[0].desc
			}else{
				middle="";
			}
        ...
        if (!forceOff && this.opt('navLinks')) {
        	if(this.name=="month" && this.opt('nongli')){
        		return '<div style="cursor:pointer;text-align:center;width:100%;padding:0px;height:25px;"><span style="float:left;padding-left:5px;font-size:16px;">'+strs+'</span><span style="color:#ff3600;font-size:10px;padding-top:1px;padding-top:0px;float:left;"></span>'+'<span style="display:inline-block;padding-top:5px;">'+middle+'</span>'+'<span style="float:right;padding-right:5px;font-size:16px;">'+ldate+'</span></div>';
        	}else{
        		return '<a' + attrs +
                ' data-goto="' + util_1.htmlEscape(JSON.stringify(finalOptions)) + '">' +
                innerHtml +
        	...
        	if(this.name=="month" && this.opt('nongli') && (''+innerHtml).indexOf("周")<=-1){
        		return '<div style="cursor:pointer;text-align:center;width:100%;padding:0px;height:25px;"><span style="float:left;padding-left:5px;font-size:16px;">'+innerHtml+'日'+'</span><span style="color:#ff3600;font-size:10px;padding-top:1px;padding-top:0px;float:left;"></span>'+'<span style="display:inline-block;padding-top:5px;">'+middle+'</span>'+'<span style="float:right;padding-right:5px;font-size:16px;">'+ldate+'</span></div>';
        	}else{
        		return '<span' + attrs + ' >' +
        		innerHtml +
        	'</span>';
