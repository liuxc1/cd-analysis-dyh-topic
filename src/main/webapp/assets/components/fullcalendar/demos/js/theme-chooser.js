...
function initThemeChooser(settings) {
	...
	function generateStylesheetUrl(themeSystem, themeName) {
		if (themeSystem === 'jquery-ui') {
			...
			if (themeName) {
				return ctx+'/assets/components/fullcalendar/css/bootstrap3/' + themeName + '/bootstrap.min.css';
			}
			else { // the default bootstrap theme
				return ctx+'/assets/components/fullcalendar/css/bootstrap3/default/bootstrap.min.css';
			}
		}
	}
