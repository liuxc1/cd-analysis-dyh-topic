let vue = new Vue({
    ...
    data: {
        url: {
            ...
            //气象要素
            getWeatherData: ctx + '/analysis/air/weatherElementChange/getWeatherData2.vm',
            //模型预报数据
            getModelForcatsData: ctx + "//analysis/forecast/multiFactorComparison/getModelForcatsData.vm",
            //分页数据
            //小时数据
            ...
            //获取污染物
            getPollutes: ctx + '/analysis/forecast/multiFactorComparison//getPollutes.vm'
        },
        //预报时长
        stepDay: '14',
        //模式信息列表
    ...
    methods: {
         ...
         * 获取污染物信息
         */
        getPolluteList: async function () {
            let _this = this;
            await AjaxUtil.sendAjaxRequest(_this.url.getPollutes, null, {}, function (json) {
                for (let i = 0; i < json.data.length; i++) {
                    let tableElement = json.data[i].POLLUTE;
                    if (tableElement === "PM25") {
                        json.data[i].OTHERNAME = "PM₂.₅"
                    } else if (tableElement === "O3_8") {
                        json.data[i].OTHERNAME = "O₃_8";
                    } else if (tableElement === "NO2") {
                        json.data[i].OTHERNAME = "NO₂";
                    } else if (tableElement === "SO2") {
                        json.data[i].OTHERNAME = "SO₂";
                    } else if (tableElement === "PM10") {
                        json.data[i].OTHERNAME = "PM₁₀";
                    } else if (tableElement === "O3") {
                        json.data[i].OTHERNAME = "O₃";
                    }
                }
                _this.polluteList = json.data;
            });
        },
        /**
         * 获取模型名称
         ...
         * echarts数据
         */
        queryModelForcastList: function () {
            let _this = this;
            let param = _this.queryParams;
            param.step = _this.stepDay;
            AjaxUtil.sendAjaxRequest(_this.url.getModelForcatsData, null, param, function (result) {
                //当天数据
                let modelForcastDataList_00 = result.data.zero;
                let modelForcastDataList_12 = result.data.twelve;
                //上一天数据
                //let oldModelForcastDataList_00 = result.data.oldZero;
                //let oldModelForcastDataList_12 = result.data.oldTwelve;
                let option_12 = {};
                let option_00 = {};

               option_12 = _this.loadModelForcastEcharts("12", modelForcastDataList_12);
               option_00 = _this.loadModelForcastEcharts("00", modelForcastDataList_00);

                _this.drawModelForcastChart(option_12, option_00)
            });
        },
        /**
         * echarts数据
         ...
         * 绘制ECharts图
         */
        drawModelForcastChart(option_12, option_00) {
            let _this = this;
            //销毁
            if (_this.echarts22) {
                _this.echarts22.dispose()
            }
            if (_this.echarts00) {
                _this.echarts00.dispose()
            }
            _this.echarts00 = echarts.init(document.getElementById('echarts00'));
            _this.echarts00.setOption(option_00);

            // 为echarts对象加载数据
            if (this.queryParams.model !== 'CFS' && _this.queryParams.model !== 'CMA') {
                _this.echarts22 = echarts.init(document.getElementById('echarts22'));
                _this.echarts22.setOption(option_12);
            }

            if (_this.echarts00 && _this.echarts22) {
                //联动配置
                echarts.connect([_this.echarts22, _this.echarts00]);
                //页面宽度变化的时候resize
                window.addEventListener("resize", function () {
                    _this.echarts00.resize();
                    _this.echarts22.resize();
                });
            } else {
                //页面宽度变化的时候resize
                window.addEventListener("resize", function () {
           ...
           /* let weatherData = [];
            ...
            for (let i = 0; i < this.modelList.length; i++) {
                ...
                for (let j = 0; j < data.length; j++) {
                    if(this.modelList[i].code === data[j]["model"]){
                        temp.push(data[j][_this.targetTypeList[targetIndex].targetCode]);
                    }
                }
                seriesData.push({
         ...
         * 加载echarts--通用
         */
        loadModelForcastEcharts: function (type, data,oldData) {
            let _this = this;
            let pollutant = _this.queryParams.pollutant;
            //获取title等属性
            let result = _this.getModelForcastEchartsData(type, data, oldData);
        ...
        loadWindEcharts: function (type, data, oldData) {
            ...
            for (let i = 0; i < this.modelList.length; i++) {
                ...
                for (let j = 0; j < data.length; j++) {
                    if( this.modelList[i].code === data[j]['model']){
                        let windSpeed = Math.ceil(data[j].windSpeed / 0.514);
                        let windDirection = data[j].windDirection;
                        let symbol = '';
                        if (windSpeed < 11) {
                        ...
                        let temp = {
                            value: data[j].windSpeed,
                            symbol: symbol,
                            symbolSize: 20,
                            symbolRotate: windDirection + 180,
                            windDirection: windDirection,
                            modelTime: data[j].modelTime.substring(0, 13),
                            resultTime: data[j].resultTime.substring(0, 13),
                            windSpeed: data[j].windSpeed
                        }
                        tempArray.push(temp);
                    }
                }
            ...
            let option = {
                ...
                tooltip: {
                    ...
                    formatter: function (params) {
                        ...
                        let windDirection0 = '';
                        if (params && params[0]) {
                            modelTime0 = params[0].seriesName;
                            windSpeed0 = '\xa0风速：' + params[0].data.windSpeed;
                            windDirection0 = '\xa0风向：' + params[0].data.windDirection;
                            arr.push(modelTime0,windSpeed0,windDirection0);
                        }
                        ...
                        let windDirection1 = '';
                        if (params && params[1]) {
                            modelTime1 = params[1].seriesName;
                            windSpeed1 = '\xa0风速：' + params[1].data.windSpeed;
                            windDirection1 = '\xa0风向：' + params[1].data.windDirection;
                            arr.push(modelTime1,windSpeed1,windDirection1);
                        }
                        ...
                        let windDirection2 = '';
                        if (params && params[2]) {
                            modelTime2 = params[2].seriesName;
                            windSpeed2 = '\xa0风速：' + params[2].data.windSpeed;
                            windDirection2 = '\xa0风向：' + params[2].data.windDirection;
                            arr.push(modelTime2,windSpeed2,windDirection2);
                        }
                        ...
                        let windDirection3 = '';
                        if (params && params[3]) {
                            modelTime3 = params[3].seriesName;
                            windSpeed3 = '\xa0风速：' + params[3].data.windSpeed;
                            windDirection3 = '\xa0风向：' + params[3].data.windDirection;
                            arr.push(modelTime3,windSpeed3,windDirection3);
                        }
                        return arr.join('<br>');
                    }
                },
        ...
        getCommonEchartsData: function (type, data, oldData) {
           ...
           /* if (oldData && oldData.length != 0) {
            ...
            for (let i = 0; i < this.modelList.length; i++) {
                modelTimeList.push(this.modelList[i].name);
            }
            //y轴名称
            let unit = '';
         ...
         * title，resultTimeList,modelTimeList,yName
         */
        getModelForcastEchartsData: function (type, data, oldData) {
            let _this = this;
            let pollutant = _this.queryParams.pollutant;
            let polluteUnit = this.getPolluteUnit(pollutant);
            //title
            let mainTitle = polluteUnit.name + '预报结果';
            let subTitle = '';
            //起报时间
            let modelTimeList = [];
            //预报时间
