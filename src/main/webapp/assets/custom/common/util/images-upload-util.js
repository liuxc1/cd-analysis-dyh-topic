 ...
 * @param {json} allowFileTypes 允许的文件类型
 * @param {number} maxFileNumber 最大允许上传文件个数
 * @link 使用之前应该先添加date-time-util.js
 * @link 使用之前应该先添加ajax-util.js
 * @link 使用之前应该先添加dialog-util.js
 * @link 使用之前应该先添加element-ui依赖
 */
function ImagesUploadUtil (maxFileSize, allowFileTypes, maxFileNumber) {
	/**
	 * 文件类型
	 */
	this.fileTypes = [{
	...
	this.renderSize = function (value) {
		...
		var decimalPoint = String(size).indexOf("."); // 获取小数点的位置
		// size = decimalPoint < 0 ? size : size.toFixed(2);
		size = size.toFixed(2);
	...
	/**
	 * 获取文件类型
	 * @param {String} rawType 文件原始类型
	 * @returns 文件常规类型
	 */
	this.getFileType = function (rawType) {
		for (var i = 0; i < this.allowFileTypes.length; i++) {
	 ...
	 * 是否超过最大文件个数
	 * @param {Array} fileList 文件列表
	 * @param {Number} maxFileNumber 最大文件个数限制
	...
	this.isExceedMaxFileNumber = function (fileList, maxFileNumber) {
		return fileList != null 
			&& fileList.length > 0 
			&& maxFileNumber > 0 
			&& fileList.length + 1 > maxFileNumber;
	};
	 ...
	 * @param {Array} fileList 文件列表
	 * @param {function} callback 验证通过的回调函数
	 */
	...
	/**
	 * 删除文件
	 * @param {Array} fileList 文件列表（和java一样，引用传递）
	 * @param {int} index 文件索引
	 * @param {Array} deleteFileIds 删除文件列表ID集合，用于记录被删除的文件ID
	 */
