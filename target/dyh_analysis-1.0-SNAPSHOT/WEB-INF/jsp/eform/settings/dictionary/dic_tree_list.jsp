...
<body class="no-skin">
					...
					<div class="row">
						<div class="col-xs-12">
							<form class="form-horizontal" role="form" id="formList"
							   ...
							   <input  type="hidden" id="form[jdpAppCode]" name="form[jdpAppCode]" value="<%=ths.jdp.core.context.PropertyConfigure.getProperty("jdp.app.code").toString()%>">
								...
								<table id="listTable" class="table table-bordered table-hover">
									<thead>
										<tr>
											...
											<th class="" data-sort-col="TREE_CODE" style="min-width:150px">
												字典编码
												<i class="ace-icon fa fa-sort pull-right"></i>
											</th>
											<th class="" data-sort-col="TREE_NAME" style="min-width:150px">
												字典名称
												<i class="ace-icon fa fa-sort pull-right"></i>
											</th>
											...
											<th class="hidden-xs hidden-sm" data-sort-col="TREE_SORT" style="min-width:150px">
												排序 
												<i class="ace-icon fa fa-sort pull-right"></i>
											</th>
									...
									<tbody>
										<c:forEach var="item" items="${pageInfo.list}">
											<tr>
												...
												<td class="hidden-xs">${item.TREE_DESCRIPTION}</td>
												<td class="hidden-xs hidden-sm">${item.TREE_SORT}</td>
												<td class="align-center col-op-ths">
													<button type="button"
														class="btn btn-sm  btn-white btn-op-ths"
														title="编辑" data-self-js="doEdit('${item.TREE_ID}')">
	...
	<script type="text/javascript">
		...
		function doAdd(){
			...
			var url="${ctx}/eform/dictree/edit.vm?table['JDP_EFORM_DICTIONARYTREE'].key['TREE_ID']="+uuid+"&dic_tree_pid=${form.treeid}";
			$("#main-container").hide();
	        $("#iframeInfo").attr("src", ths.urlEncode4Get(url)).show();
		}
		
		function doEdit(id){
			var url="${ctx}/eform/dictree/edit.vm?table['JDP_EFORM_DICTIONARYTREE'].key['TREE_ID']="+id+"&dic_tree_pid=${form.treeid}";
			$("#main-container").hide();
	        $("#iframeInfo").attr("src", ths.urlEncode4Get(url)).show();
		}
		...
		function doDelete() {
			...
			__doDelete(_ids, "delete.vm", function() {
				if(window.parent.expandNode){
					window.parent.expandNode("${form.treeid}");
				}
				//刷出之后，刷新列表
				doSearch();
			});
